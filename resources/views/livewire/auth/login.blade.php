...
new #[Layout('components.layouts.auth')] class extends Component {
    ...
    public function login(): void
    {
        ...
        } elseif (in_array($user->role, ['umum', 'peserta'])) {
            $this->redirect(route('user.home'));
        } else {
            Auth::logout();
...
<div>
    ...
    <form wire:submit="login" class="mb-6" id="login-form">
        ...
        <!-- Tambah sebelum tombol login -->
        {{-- {!! NoCaptcha::renderJs() !!}
        {!! NoCaptcha::display() !!} --}}

        {{-- <script>
            function setCaptchaToken(token) {
                window.livewire.first().set('captcha', token);
            }
        </script> --}}

        <!-- Tambahkan setelah semua field input sebelum tombol login -->
{!! NoCaptcha::renderJs() !!}
...
<script>
    function onCallback(token) {
        // misalnya:
        window.Livewire.find('{{ $this->getId() }}').set('captcha', token);
    }
</script>
